...
def check_stability(best_cfg, param_name, test_values, days_ago_start, days_ago_end):
    ...
    if is_stable:
        print(f"   ✅ STABLE (CV < 0.15)")
    else:
        print(f"   ⚠️  UNSTABLE (CV >= 0.15) - may be overfit!")
...
def optimize_group(name, configs, train_start, train_end, test_start, test_end, test_costs_multiplier=1.0):
    ...
    if train_sharpe > 0:
        ...
        if abs(decay) < 10:
            print(f"  ✅ EXCELLENT: Stable performance (<10% decay)")
        elif abs(decay) < 25:
            print(f"  ✅ GOOD: Acceptable decay (<25%)")
        elif decay > 30:
            print(f"  ⚠️  WARNING: High decay (>{30}%) suggests overfitting!")
        else:
            print(f"  ✅ AMAZING: System improves OOS!")
